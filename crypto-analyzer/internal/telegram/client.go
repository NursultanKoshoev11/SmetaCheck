...
func New(token, chatID string, timeout time.Duration) *Client {
	return &Client{
		token:      strings.TrimSpace(token),
		chatID:     strings.TrimSpace(chatID),
		baseURL:    "https://api.telegram.org",
		httpClient: &http.Client{Timeout: timeout + 40*time.Second},
	}
}
...
func (c *Client) Send(ctx context.Context, text string) (int, error) {
	if !c.Enabled() {
	...
	for _, chunk := range chunks {
		...
		resp.Body.Close()
		if readErr != nil {
			return lastCode, readErr
		}
		var result apiResponse[json.RawMessage]
		if err := json.Unmarshal(responseBody, &result); err != nil {
			return lastCode, fmt.Errorf("decode Telegram response: %w", err)
		}
		if resp.StatusCode < 200 || resp.StatusCode >= 300 || !result.OK {
			return lastCode, fmt.Errorf("Telegram API error %d: %s", result.ErrorCode, result.Description)
		}
	}
	return lastCode, nil
}
...
func (c *Client) GetUpdates(ctx context.Context, offset int64) ([]Update, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 || !result.OK {
		return nil, fmt.Errorf("Telegram getUpdates error %d: %s", result.ErrorCode, result.Description)
	}
