...
func Validate(path, originalName string, limits Limits) (Result, error) {
	...
	case ".pdf":
		...
		return result, nil
	default:
		return Result{}, fmt.Errorf("unsupported file format: use XLSX, XLSM, CSV or PDF")
	}
}

func normalizeLimits(l Limits) Limits {
	d := DefaultLimits()
	if l.MaxArchiveFiles <= 0 { l.MaxArchiveFiles = d.MaxArchiveFiles }
	if l.MaxUncompressedBytes <= 0 { l.MaxUncompressedBytes = d.MaxUncompressedBytes }
	if l.MaxEntryBytes <= 0 { l.MaxEntryBytes = d.MaxEntryBytes }
	if l.MaxCompressionRatio <= 0 { l.MaxCompressionRatio = d.MaxCompressionRatio }
	if l.MaxNestedArchives < 0 { l.MaxNestedArchives = d.MaxNestedArchives }
	if l.InspectionTimeout <= 0 { l.InspectionTimeout = d.InspectionTimeout }
	return l
}

func digestAndPrefix(path string) (string, []byte, error) {
	f, err := os.Open(path)
	if err != nil { return "", nil, fmt.Errorf("open uploaded file: %w", err) }
	defer f.Close()
	h := sha256.New()
	...
	prefix = prefix[:n]
	_, _ = h.Write(prefix)
	if _, err := io.Copy(h, f); err != nil { return "", nil, fmt.Errorf("hash uploaded file: %w", err) }
...
func validateCSV(path string, prefix []byte) error {
	...
	if err != nil { return fmt.Errorf("inspect CSV: %w", err) }
	control := 0
...
func validateOfficeArchive(path, ext string, limits Limits) (bool, string, error) {
	...
	if err != nil { return false, "", fmt.Errorf("invalid Office archive: %w", err) }
	defer reader.Close()
	...
	for _, entry := range reader.File {
		if time.Now().After(deadline) { return false, "", fmt.Errorf("Office archive inspection timed out") }
		...
		if entry.UncompressedSize64 > 0 {
			if entry.CompressedSize64 == 0 { return false, "", fmt.Errorf("Office archive contains an invalid compressed entry") }
		...
		if lower == "xl/workbook.xml" { hasWorkbook = true }
		if lower == "xl/vbaproject.bin" { hasMacros = true }
		...
		if lower == "[content_types].xml" {
			...
			if err != nil { return false, "", fmt.Errorf("read Office content types: %w", err) }
		}
	}
	...
	if strings.Contains(content, "application/vnd.ms-office.vbaproject") { hasMacros = true }
	if ext == ".xlsx" {
...
func readZipEntryLimited(entry *zip.File, max int64) ([]byte, error) {
	...
	if err != nil { return nil, err }
	if int64(len(data)) > max { return nil, fmt.Errorf("entry is too large") }
	return data, nil
}
...
func ScanClamAV(ctx context.Context, path, address string, maxBytes int64) error {
	...
	if address == "" { return fmt.Errorf("ClamAV address is not configured") }
	if maxBytes <= 0 { maxBytes = 100 * 1024 * 1024 }
	var dialer net.Dialer
	conn, err := dialer.DialContext(ctx, "tcp", address)
	if err != nil { return fmt.Errorf("connect to ClamAV: %w", err) }
	defer conn.Close()
	if deadline, ok := ctx.Deadline(); ok { _ = conn.SetDeadline(deadline) }
	if _, err := conn.Write([]byte("zINSTREAM\x00")); err != nil { return fmt.Errorf("start ClamAV scan: %w", err) }
	f, err := os.Open(path)
	if err != nil { return fmt.Errorf("open file for antivirus scan: %w", err) }
	defer f.Close()
	...
	for {
		...
		if n > 0 {
			sent += int64(n)
			if sent > maxBytes { return fmt.Errorf("file exceeds antivirus scan limit") }
			var size [4]byte
			binary.BigEndian.PutUint32(size[:], uint32(n))
			if _, err := conn.Write(size[:]); err != nil { return fmt.Errorf("stream size to ClamAV: %w", err) }
			if _, err := conn.Write(buf[:n]); err != nil { return fmt.Errorf("stream file to ClamAV: %w", err) }
		}
		if readErr == io.EOF { break }
		if readErr != nil { return fmt.Errorf("read file for ClamAV: %w", readErr) }
	}
	if _, err := conn.Write([]byte{0, 0, 0, 0}); err != nil { return fmt.Errorf("finish ClamAV stream: %w", err) }
	response, err := io.ReadAll(io.LimitReader(conn, 4096))
	if err != nil { return fmt.Errorf("read ClamAV response: %w", err) }
	text := strings.TrimSpace(strings.TrimRight(string(response), "\x00"))
	if strings.Contains(text, "FOUND") { return fmt.Errorf("%w: %s", ErrMalwareDetected, text) }
	if !strings.HasSuffix(text, "OK") { return fmt.Errorf("unexpected ClamAV response: %s", text) }
	return nil
}
