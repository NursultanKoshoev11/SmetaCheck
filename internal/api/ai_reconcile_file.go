...
func reconcileFile(estimate Estimate, aiFile AIFileAnalysis) UnifiedFileReport {
	...
	if !isIndependentAIAnalysis(aiFile) {
		...
		for _, item := range backend {
			unified = append(unified, UnifiedFinding{
				...
				Reconciliation:  "backend_only_ai_unavailable",
			})
		}
		sortUnifiedFindings(unified)
		backendRisk := riskFromProviderFindings(backend)
		if backendRisk == "" {
			backendRisk = "Не определён"
		}
		summary := strings.TrimSpace(aiFile.Warning)
		if summary == "" {
			summary = "Независимый AI-анализ недоступен. Итог построен только по backend-проверкам."
		}
		return UnifiedFileReport{
			EstimateID:       estimate.ID,
			FileName:         estimate.FileName,
			BackendScore:     estimate.Score,
			AIRiskLevel:      "Не определён",
			FinalRiskLevel:   backendRisk,
			AgreementScore:   0,
			Summary:          summary,
			TotalAmount:      estimate.TotalAmount,
			AIExtractedTotal: 0,
			Findings:         unified,
			BackendChart:     buildRiskChart(estimate.Findings),
			AIChart:          chartFromAIFindings(nil),
			UnifiedChart:     chartFromUnifiedFindings(unified),
			Recommendations: deduplicateStrings(aiFile.Recommendations),
			AIAnalysis:       aiFile,
		}
	}
	...
	for _, item := range backend {
		...
		if match >= 0 {
			...
			unified = append(unified, UnifiedFinding{
				FileID:          estimate.ID,
				Row:             chooseRow(item.Row, other.Row),
				Category:        chooseNonEmpty(item.Category, other.Category),
				Title:           chooseNonEmpty(other.Title, item.Title),
				Severity:        maxSeverity(item.Severity, other.Severity),
				Detail:          mergeDetails(item.Detail, other.Detail),
				SuggestedAction: chooseNonEmpty(other.SuggestedAction, item.SuggestedAction),
				Sources:         []string{"backend", "ai"},
				Reconciliation:  "confirmed_by_both",
			})
			continue
		}
		unified = append(unified, UnifiedFinding{
	...
	agreementScore := 100
	if denominator > 0 {
		agreementScore = int(math.Round(float64(agreements) / float64(denominator) * 100))
	}
	coverage := 1.0
	if estimate.ItemsCount > 0 {
		coverage = math.Min(1, float64(aiFile.RowsAnalyzed)/float64(estimate.ItemsCount))
	}
	...
	if finalRisk == "" {
		finalRisk = "Не определён"
	}
	return UnifiedFileReport{
		EstimateID:       estimate.ID,
		FileName:         estimate.FileName,
		BackendScore:     estimate.Score,
		AIRiskLevel:      normalizeRiskLevel(aiFile.RiskLevel),
		FinalRiskLevel:   finalRisk,
		AgreementScore:   agreementScore,
		Summary:          aiFile.Summary,
		TotalAmount:      estimate.TotalAmount,
		AIExtractedTotal: aiFile.ExtractedTotal,
		Findings:         unified,
		BackendChart:     buildRiskChart(estimate.Findings),
		AIChart:          chartFromAIFindings(aiFile.Findings),
		UnifiedChart:     chartFromUnifiedFindings(unified),
		Recommendations: deduplicateStrings(aiFile.Recommendations),
		AIAnalysis:       aiFile,
	}
...
func bestAIMatch(backend AIProviderFinding, ai []AIProviderFinding, used []bool) int {
	...
	for index, candidate := range ai {
		...
		score := 0
		if backend.Row > 0 && candidate.Row == backend.Row {
			score += 5
		}
		if strings.EqualFold(strings.TrimSpace(backend.Category), strings.TrimSpace(candidate.Category)) {
			score += 4
		}
		if tokenOverlap(backend.Title+" "+backend.Detail, candidate.Title+" "+candidate.Detail) >= 0.35 {
			score += 3
		}
		if score > bestScore {
...
func mergeDetails(a, b string) string {
	if strings.TrimSpace(a) == strings.TrimSpace(b) {
		return strings.TrimSpace(a)
	}
	return strings.TrimSpace(a) + " AI: " + strings.TrimSpace(b)
}
