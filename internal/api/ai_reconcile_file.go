...
func reconcileFile(estimate Estimate, aiFile AIFileAnalysis) UnifiedFileReport {
	...
	sort.SliceStable(unified, func(i, j int) bool {
		left, right := severityRank(unified[i].Severity), severityRank(unified[j].Severity)
		if left != right { return left > right }
		return unified[i].Row < unified[j].Row
	})
	...
	return UnifiedFileReport{
		...
		AgreementScore: agreementScore, Summary: aiFile.Summary,
		TotalAmount: estimate.TotalAmount, AIExtractedTotal: aiFile.ExtractedTotal,
		Findings: unified, BackendChart: buildRiskChart(estimate.Findings),
		AIChart: chartFromAIFindings(aiFile.Findings), UnifiedChart: chartFromUnifiedFindings(unified),
...
func bestAIMatch(backend AIProviderFinding, ai []AIProviderFinding, used []bool) int {
	...
	for index, candidate := range ai {
		...
		score := 0
		if backend.Row > 0 && candidate.Row == backend.Row { score += 5 }
		if strings.EqualFold(strings.TrimSpace(backend.Category), strings.TrimSpace(candidate.Category)) { score += 4 }
		if tokenOverlap(backend.Title+" "+backend.Detail, candidate.Title+" "+candidate.Detail) >= 0.35 { score += 3 }
