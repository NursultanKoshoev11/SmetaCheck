...
func RunProduction() {
	...
	mux.HandleFunc("/v1/admin/",AdminManagementRouter)
	...
	mux.HandleFunc("/v1/estimates/",EstimateRouterAudited)
	...
	select{
	case err:=<-serveErr:
		closeDatabase();if err!=nil&&!errors.Is(err,http.ErrServerClosed){log.Fatalf("api server failed: %v",err)}
	case <-rootCtx.Done():
		shutdownTimeout:=envDuration("SERVER_SHUTDOWN_TIMEOUT",20*time.Second);if shutdownTimeout<time.Second{shutdownTimeout=20*time.Second}
		shutdownCtx,cancel:=context.WithTimeout(context.Background(),shutdownTimeout);shutdownErr:=server.Shutdown(shutdownCtx);cancel();listenerErr:=<-serveErr;closeDatabase()
