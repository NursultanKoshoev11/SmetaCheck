...
func currentRequestUser(r *http.Request) (RequestUser, bool) {
	...
	if tokenText == "" { return RequestUser{}, false }
	secret := strings.TrimSpace(os.Getenv("JWT_SECRET"))
	if secret == "" { return RequestUser{}, false }
	claims := jwt.MapClaims{}
	token, err := jwt.ParseWithClaims(tokenText, claims, func(token *jwt.Token) (any, error) {
		if token.Method != jwt.SigningMethodHS256 { return nil, fmt.Errorf("unexpected signing method") }
		return []byte(secret), nil
	}, jwt.WithIssuer(authIssuer()), jwt.WithAudience("smetacheck-api"), jwt.WithExpirationRequired())
	if err != nil || !token.Valid { return RequestUser{}, false }
	user := RequestUser{}
	...
	if user.ID == "" || user.SessionID == "" { return RequestUser{}, false }
	pool, err := getDB(r.Context())
	if err != nil || pool == nil { return RequestUser{}, false }
	var active bool
	err = pool.QueryRow(r.Context(), `
		SELECT (s.revoked_at IS NULL AND s.expires_at>now() AND u.disabled_at IS NULL),
		       COALESCE(u.role,'owner'), COALESCE(u.mfa_enabled,false), (s.mfa_verified_at IS NOT NULL)
		FROM auth_sessions s JOIN users u ON u.id=s.user_id
		WHERE s.id=$1 AND s.user_id=$2
	`, user.SessionID, user.ID).Scan(&active, &user.Role, &user.MFAEnabled, &user.MFAVerified)
	if err != nil || !active { return RequestUser{}, false }
	return user, true
}
...
func requireUserForProduction(w http.ResponseWriter, r *http.Request) (RequestUser, bool) {
	...
	if user.Role == "admin" && (!user.MFAEnabled || !user.MFAVerified) {
		estimateWriteError(w, http.StatusForbidden, "administrator MFA verification required")
		return RequestUser{}, false
	}
	return user, true
}
