...
func generateAISummary(ctx context.Context, ownerID string, estimate Estimate) AISummaryResponse {
	...
	if provider.Name() != "rules" {
		if err := reserveAccountUsage(ctx, ownerID, UsageDelta{AIJobs: 1}); err != nil {
			fallback := buildAISummary(estimate)
			if quotaErr, ok := err.(*UsageQuotaError); ok {
				fallback.Warning = fmt.Sprintf("AI quota exceeded: %s (%d/%d used).", quotaErr.Resource, quotaErr.Used, quotaErr.Limit)
			} else {
				fallback.Warning = "AI usage accounting is temporarily unavailable."
				log.Printf("cannot reserve AI usage estimate_id=%s: %v", estimate.ID, err)
			}
			return fallback
		}
	}
...
func summaryFromAIFile(estimate Estimate, file AIFileAnalysis, provider, model string) AISummaryResponse {
	...
	if len(keyRisks) > 10 {
		keyRisks = keyRisks[:10]
	}
	if len(costFlags) > 10 {
		costFlags = costFlags[:10]
	}
	if len(questions) > 10 {
...
func buildAISummary(estimate Estimate) AISummaryResponse {
	...
	if high > 0 {
		priority = append(priority, "Исправить все High-замечания.")
	}
	if medium > 0 {
		priority = append(priority, "Проверить все Medium-замечания.")
	}
...
func ruleBasedBrief(estimate Estimate, high, medium, dataQuality int) string {
	if estimate.ItemsCount == 0 {
		return "Система не смогла распознать позиции сметы."
	}
	brief := fmt.Sprintf("Распознано %d позиций на сумму %.2f сом.", estimate.ItemsCount, estimate.TotalAmount)
	if high > 0 {
		brief += fmt.Sprintf(" High: %d.", high)
	} else if medium > 0 {
		brief += fmt.Sprintf(" Medium: %d.", medium)
	} else {
		brief += " Критичных рисков не найдено."
	}
	if dataQuality < 70 {
		brief += " Качество данных низкое."
	}
	return brief
}
...
func buildRuleQuestions(estimate Estimate) []string {
	...
	for _, finding := range estimate.Findings {
		if finding.Severity == "High" || finding.Severity == "Medium" {
			result = append(result, finding.Title+": "+finding.Detail)
			if len(result) == 6 {
				break
			}
		}
	...
	if len(result) == 0 {
		result = append(result, "Совпадает ли итоговая сумма с договором и объёмом работ?")
	}
	return result
}
