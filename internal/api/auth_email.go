...
func AuthRegisterEmail(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.FullName) < 2 || len(req.FullName) > 120 {
		estimateWriteError(w, http.StatusBadRequest, "full name must be between 2 and 120 characters")
		return
	}
	...
	if err != nil {
		estimateWriteError(w, http.StatusInternalServerError, "cannot secure password")
		return
	}
	...
	if err != nil {
		estimateWriteError(w, http.StatusInternalServerError, "cannot start registration")
		return
	}
	...
	`, newDatabaseID("idn"), user.ID, user.Email)
	if err != nil {
		estimateWriteError(w, http.StatusInternalServerError, "cannot save email identity")
		return
	}
	token, err := createOneTimeTokenTx(r.Context(), tx, user.ID, "verify_email", 24*time.Hour)
	if err != nil {
		estimateWriteError(w, http.StatusInternalServerError, "cannot create verification token")
		return
	}
	if err := tx.Commit(r.Context()); err != nil {
		estimateWriteError(w, http.StatusInternalServerError, "cannot complete registration")
		return
	}
	if err := sendVerificationEmail(user, token); err != nil {
		estimateWriteError(w, http.StatusServiceUnavailable, "account created but verification email could not be sent; use resend verification")
		return
	}
...
func AuthLoginEmail(w http.ResponseWriter, r *http.Request) {
	...
	err = pool.QueryRow(r.Context(), `
		SELECT id,COALESCE(email,''),COALESCE(full_name,''),COALESCE(avatar_url,''),
		       COALESCE(password_hash,''),email_verified_at,failed_login_attempts,
		       locked_until,created_at
		FROM users WHERE lower(email)=lower($1)
	`, email).Scan(&user.ID,&user.Email,&user.FullName,&user.AvatarURL,&user.PasswordHash,
		&user.EmailVerifiedAt,&failed,&lockedUntil,&user.CreatedAt)
	...
	if lockedUntil != nil && lockedUntil.After(time.Now().UTC()) {
		estimateWriteError(w, http.StatusTooManyRequests, "account is temporarily locked; try again later")
		return
	}
	if user.PasswordHash == "" || bcrypt.CompareHashAndPassword([]byte(user.PasswordHash), []byte(req.Password)) != nil {
		...
		if failed >= maxLoginAttempts {
			_, _ = pool.Exec(r.Context(), `UPDATE users SET failed_login_attempts=0,locked_until=$1,updated_at=now() WHERE id=$2`, time.Now().UTC().Add(loginLockDuration), user.ID)
		} else {
			_, _ = pool.Exec(r.Context(), `UPDATE users SET failed_login_attempts=$1,updated_at=now() WHERE id=$2`, failed, user.ID)
		}
		estimateWriteError(w, http.StatusUnauthorized, "invalid email or password")
		return
	}
	if user.EmailVerifiedAt == nil {
		estimateWriteError(w, http.StatusForbidden, "email verification is required")
		return
	}
	...
	if err := createBrowserSession(w, r, user); err != nil {
		estimateWriteError(w, http.StatusInternalServerError, "cannot create session")
		return
	}
...
func AuthVerifyEmail(w http.ResponseWriter, r *http.Request) {
	...
	if token == "" {
		...
		if err := decodeJSONBody(r, &req); err == nil { token = strings.TrimSpace(req.Token) }
	}
	userID, err := consumeOneTimeToken(r.Context(), "verify_email", token)
	if err != nil {
		estimateWriteError(w, http.StatusBadRequest, "verification link is invalid or expired")
		return
	}
	...
	if err != nil {
		estimateWriteError(w, http.StatusInternalServerError, "cannot verify email")
		return
	}
...
func AuthResendVerification(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil && user.EmailVerifiedAt == nil {
		token, tokenErr := createOneTimeToken(r.Context(), user.ID, "verify_email", 24*time.Hour)
		if tokenErr == nil { _ = sendVerificationEmail(user, token) }
	}
	estimateWriteJSON(w, http.StatusAccepted, map[string]bool{"accepted": true})
}
	...
	if err := decodeJSONBody(r, &req); err == nil {
		if email, normalizeErr := normalizeEmail(req.Email); normalizeErr == nil {
			...
			err := pool.QueryRow(r.Context(), `SELECT id,email,COALESCE(full_name,''),COALESCE(avatar_url,''),email_verified_at,created_at FROM users WHERE lower(email)=lower($1) AND password_hash IS NOT NULL`, email).
				Scan(&user.ID,&user.Email,&user.FullName,&user.AvatarURL,&user.EmailVerifiedAt,&user.CreatedAt)
			if err == nil {
				token, tokenErr := createOneTimeToken(r.Context(), user.ID, "reset_password", time.Hour)
				if tokenErr == nil { _ = sendPasswordResetEmail(user, token) }
			}
		}
	}
	estimateWriteJSON(w, http.StatusAccepted, map[string]bool{"accepted": true})
...
func AuthResetPassword(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		estimateWriteError(w, http.StatusBadRequest, "reset link is invalid or expired")
		return
	}
	hash, err := bcrypt.GenerateFromPassword([]byte(req.Password), passwordHashCost)
	if err != nil {
	...
	if err != nil {
		estimateWriteError(w, http.StatusInternalServerError, "cannot reset password")
		return
	}
...
func createOneTimeToken(ctx context.Context, userID, purpose string, ttl time.Duration) (string, error) {
	...
	if err != nil { return "", err }
	_, err = pool.Exec(ctx, `DELETE FROM auth_tokens WHERE user_id=$1 AND purpose=$2 AND consumed_at IS NULL`, userID, purpose)
	if err != nil { return "", err }
...
func consumeOneTimeToken(ctx context.Context, purpose, token string) (string, error) {
	...
	err = pool.QueryRow(ctx, `
		...
		RETURNING user_id
	`, hashToken(token), purpose).Scan(&userID)
	return userID, err
}

func validatePassword(password string) error {
	if len(password) < 12 { return fmt.Errorf("password must be at least 12 characters") }
	if len(password) > 128 { return fmt.Errorf("password must be at most 128 characters") }
	return nil
}
...
func redirectAuthResult(w http.ResponseWriter, r *http.Request, path string) {
	base := strings.TrimRight(strings.TrimSpace(os.Getenv("PUBLIC_BASE_URL")), "/")
	if base == "" { estimateWriteJSON(w, http.StatusOK, map[string]bool{"ok": true}); return }
