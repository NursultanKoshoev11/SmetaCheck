...
export default function Upload(){
  ...
  async function submitUpload(){
    ...
    try{
      ...
      setResult(data);
      setStatus('done');
      setMessage('Смета загружена и сохранена в PostgreSQL.');
    }catch(error){
      setStatus('error');
    ...
    <main className="page">
      ...
      <section className="workspace">
        ...
        {!checkingSession && authorized && <div className="twoColumns">
          ...
          <div className="card checklistCard"><h2>Что получает клиент</h2><ul><li>Смета привязана к аккаунту</li><li>Позиции и замечания сохраняются в PostgreSQL</li><li>Создаётся история проверок</li><li>AI объясняет риски простым языком</li><li>Отчёт можно сохранить как PDF</li></ul><a className="btn secondary" href="/dashboard">Открыть кабинет</a></div>
        </div>}
      </section>
      <Footer/>
    </main>
